...
abstract class CallxReactActivity : ReactActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ensure we show over lock screen when launched from an incoming call
        if (fromIncomingCall && deviceWasLocked) {
            try {
                ...
                Log.d(TAG, "✅ Enabled show over lock screen in CallxReactActivity")
            } catch (e: Exception) {
                Log.w(TAG, "⚠️ Failed to enable show over lock screen: ${e.message}")
            }
        }
    ...
    override fun onNewIntent(intent: Intent?) {
        ...
        if (fromIncomingCall && deviceWasLocked) {
            try {
                ...
                Log.d(TAG, "✅ Enabled show over lock screen in CallxReactActivity (onNewIntent)")
            } catch (e: Exception) {
                Log.w(TAG, "⚠️ Failed to enable show over lock screen (onNewIntent): ${e.message}")
            }
        }
