...
- (void)setupCallKit {
    ...
    // Check call logging configuration
    NSDictionary *callLoggingConfig = self.configuration[@"callLogging"];
    BOOL callLoggingEnabled = callLoggingConfig ? [callLoggingConfig[@"enabled"] boolValue] : YES;
    
    // Enable call history logging based on configuration
...
- (void)handleCallAnsweredElsewhere:(NSDictionary *)callData {
    ...
    // Log additional info
    NSString *answeredBy = callData[@"answeredBy"];
    NSString *deviceType = callData[@"deviceType"];
    
    if (answeredBy) {
        RCTLogInfo(@"CallxPushKitHandler: Answered by: %@", answeredBy);
    }
    if (deviceType) {
        RCTLogInfo(@"CallxPushKitHandler: Device type: %@", deviceType);
    }
}

// Handle call declined
- (void)handleCallDeclined:(NSDictionary *)callData {
    RCTLogInfo(@"CallxPushKitHandler: Call declined - %@", callData[@"callId"]);
...
// Handle call timeout
- (void)handleCallTimeout:(NSDictionary *)callData {
    RCTLogInfo(@"CallxPushKitHandler: Call timeout - %@", callData[@"callId"]);
...
- (void)handleCallCancelled:(NSDictionary *)callData {
    RCTLogInfo(@"CallxPushKitHandler: Call cancelled - %@", callData[@"callId"]);
...
- (void)handleCallBusy:(NSDictionary *)callData {
    RCTLogInfo(@"CallxPushKitHandler: Call busy - %@", callData[@"callId"]);
...
- (NSDictionary *)parseCallDataFromPush:(NSDictionary *)pushData {
    ...
    NSString *hasVideoString = [self getFieldValue:pushData fieldConfig:fields[@"hasVideo"]];
    NSString *endReason = [self getFieldValue:pushData fieldConfig:fields[@"endReason"]];
    NSString *answeredBy = [self getFieldValue:pushData fieldConfig:fields[@"answeredBy"]];
    NSString *deviceType = [self getFieldValue:pushData fieldConfig:fields[@"deviceType"]];
    NSString *durationString = [self getFieldValue:pushData fieldConfig:fields[@"duration"]];
    ...
    BOOL hasVideo = NO;
    if (hasVideoString) {
        hasVideo = [hasVideoString boolValue];
    }
...
- (void)pushRegistry:(PKPushRegistry *)registry didReceiveIncomingPushWithPayload:(PKPushPayload *)payload forType:(PKPushType)type withCompletionHandler:(void (^)(void))completion {
    if ([type isEqualToString:PKPushTypeVoIP]) {
        ...
        if ([triggerType isEqualToString:@"incoming"] && callData) {
            ...
            [self handleCallDeclined:callData];
            ...
            // Handle call timeout
            [self handleCallTimeout:callData];
            ...
            [self handleCallCancelled:callData];
...
- (void)provider:(CXProvider *)provider performAnswerCallAction:(CXAnswerCallAction *)action {
    ...
    NSDictionary *callLoggingConfig = self.configuration[@"callLogging"];
    BOOL logAnswered = callLoggingConfig ? [callLoggingConfig[@"logAnswered"] boolValue] : YES;
...
- (void)provider:(CXProvider *)provider performEndCallAction:(CXEndCallAction *)action {
    ...
    NSDictionary *callLoggingConfig = self.configuration[@"callLogging"];
    BOOL logDuration = callLoggingConfig ? [callLoggingConfig[@"logDuration"] boolValue] : YES;
    
    if (logDuration) {
        RCTLogInfo(@"CallxPushKitHandler: Call logged to phone history (ended)");
    } else {
        RCTLogInfo(@"CallxPushKitHandler: Call duration logging disabled");
    }
